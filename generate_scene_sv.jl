...
function write_color(im, pixel_color, samples_per_pixel)
    ...
    r = clamp(sqrt(pixel_color[1] * scale), 0.0, 0.999)
    g = clamp(sqrt(pixel_color[2] * scale), 0.0, 0.999)
    b = clamp(sqrt(pixel_color[3] * scale), 0.0, 0.999)
...
function ray_color(ray::Ray, world::HittableList, depth::Int64)
    hit_record = Hit_record(0. , Vec3([0.,0.,0.]), Vec3([0.,0.,0.]), Lambertian(), false)
    ...
    if hit(world, ray, 0.001 , Base.Inf64, hit_record)
        ...
        scat = scatter(hit_record.material, ray, hit_record, scattered)
        if scat[1]
            return scat[2] * ray_color(scattered, world, depth - 1)
        end
        return Vec3([0.,0.,0.])
    end
...
function main()
    ...
    Threads.@threads for i in 1:image_w
        ...
        Threads.@threads for j in 1:image_h
            image[i,j] = Vec3(zeros(Float64, 1, 3))
