...
struct Sphere <: Hittable
    ...
    Sphere() = new(Vec3(0.,0.,0.), 1., Lambertian())
...
function scatter(material::Material, ray_in::Ray, hit_record::Hit_record, scattered::Ray)
    return (false, Vec3([0.,0.,0.]))
end

function scatter(material::Lambertian, ray_in::Ray, hit_record::Hit_record, scattered::Ray)
    ...
    attenuation = Vec3(material.albedo[1], material.albedo[2], material.albedo[3])
...
struct Metal <: Material
    ...
    Metal() = new(Vec3(0.,0.,0.), 0.)
    Metal(albedo, fuzz) = new(albedo, fuzz)
end

function scatter(material::Metal, ray_in::Ray, hit_record::Hit_record, scattered::Ray)
    ...
    attenuation = Vec3(material.albedo[1], material.albedo[2], material.albedo[3])
    return (dot(scattered.direction, hit_record.normal) > 0, attenuation)
end
...
function scatter(material::Dielectric, ray_in::Ray, hit_record::Hit_record, scattered::Ray)
